...
public class Sparkle implements ClientModInitializer {
	...
	public void onInitializeClient() {
		SPARKLY_BLOCKS = CONFIG.blocks.stream().filter(string -> string != null && !string.isEmpty()).map(thing -> Registries.BLOCK.getOrEmpty(Identifier.of(thing))).filter(Optional::isPresent).map(Optional::get).toList();
		SPARKLY_ITEMS = CONFIG.items.stream().filter(string -> string != null && !string.isEmpty()).map(thing -> Registries.ITEM.getOrEmpty(Identifier.of(thing))).filter(Optional::isPresent).map(Optional::get).toList();
		SPARKLY_ENTITIES = CONFIG.entities.stream().filter(string -> string != null && !string.isEmpty()).map(thing -> Registries.ENTITY_TYPE.getOrEmpty(Identifier.of(thing))).filter(Optional::isPresent).map(Optional::get).toList();
